...
protocol TIPTipViewDelegate: class {
...
class TIPTipView: UIView {
    
    // MARK: - Properties
    weak var delegate: TIPTipViewDelegate?
    let shapeReuseId: String = "iro.reuseId.shape"
    
    // Add buffer space to front and back by adding blank images
    let startBuffer: TIPShape = TIPShape()
    let crown: TIPShape = TIPShape(name: "crown", minutes: 20, coins: 50)
    let flame: TIPShape = TIPShape(name: "flame", minutes: 60, coins: 150)
    let diamond: TIPShape = TIPShape(name: "diamond", minutes: 20, coins: 100)
    let heart: TIPShape = TIPShape(name: "heart", minutes: 40, coins: 200)
    let kiss: TIPShape = TIPShape(name: "kiss", minutes: 60, coins: 300)
    let smile: TIPShape = TIPShape(name: "smile", minutes: 80, coins: 400)
    let sun: TIPShape = TIPShape(name: "sun", minutes: 100, coins: 500)
    let endBuffer: TIPShape = TIPShape()
    
    lazy var shapes: [TIPShape] = [self.startBuffer, self.diamond, self.heart, self.kiss, self.smile, self.sun, self.endBuffer]
    
    // Update the labels when a new shape is closest to center
    var closestToCenterIndex: Int = 0 {
        didSet {
            let shape: TIPShape = self.shapes[closestToCenterIndex]
            if shape.name != nil {
    ...
    lazy var buyCoinsButton: TIPButton = {
    ...
    lazy var shapesCollectionView: UICollectionView = {
        let layout: TIPShapesCollectionViewFlowLayout = TIPShapesCollectionViewFlowLayout()
        layout.scrollDirection = .horizontal
        let collectionView: UICollectionView = UICollectionView(frame: .zero, collectionViewLayout: layout)
        collectionView.register(TIPShapeCollectionViewCell.self, forCellWithReuseIdentifier: self.shapeReuseId)
...
extension TIPTipView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: TIPShapeCollectionViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: self.shapeReuseId, for: indexPath) as! TIPShapeCollectionViewCell
        let shape: TIPShape = self.shapes[indexPath.item]
...
extension TIPTipView: UICollectionViewDelegate {
...
extension TIPTipView: UICollectionViewDelegateFlowLayout {
...
class TIPShapesCollectionViewFlowLayout: UICollectionViewFlowLayout {
